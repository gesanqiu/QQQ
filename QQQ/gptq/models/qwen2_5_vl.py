...
@torch.no_grad()
def gptq_qwen2_5_vl_func(model, dataloader, dev, args, force_to_cpu=False):
    """GPTQ quantization for Qwen2.5-VL, targeting only language_model layers."""
    print("Starting GPTQ quantization (Qwen2.5-VL, language-only) ...")
...
class QuantizedQwen2_5_VLMLP(Qwen2MLP):
...
class QuantizedQwen2_5_VLAttention(Qwen2_5_VLAttention):
...
class QuantizedQwen2_5_VLDecoderLayer(Qwen2_5_VLDecoderLayer):
    def __init__(self, config, quant_config, layer_idx):
        nn.Module.__init__(self)
        self.hidden_size = config.hidden_size

        self.self_attn = QuantizedQwen2_5_VLAttention(config, quant_config, layer_idx)

        self.mlp = QuantizedQwen2_5_VLMLP(config, quant_config)
...
class QuantizedQwen2_5_VLTextModel(Qwen2_5_VLTextModel):
    def __init__(self, config, quant_config):
        ...
        self.layers = nn.ModuleList([
            QuantizedQwen2_5_VLDecoderLayer(config, quant_config, i)
...
class QuantizedQwen2_5_VLModel(Qwen2_5_VLModel):
    """Quantized language model + FP vision encoder."""

    def __init__(self, config, quant_config):
        super(Qwen2_5_VLModel, self).__init__(config)
        self.visual = Qwen2_5_VisionTransformerPretrainedModel._from_config(config.vision_config)
        self.language_model = QuantizedQwen2_5_VLTextModel(config.text_config, quant_config)
        self.rope_deltas = None

        self.post_init()


class QuantizedQwen2_5_VLForConditionalGeneration(Qwen2_5_VLForConditionalGeneration):
    def __init__(self, config, quant_config):
        super(Qwen2_5_VLForConditionalGeneration, self).__init__(config)
        self.model = QuantizedQwen2_5_VLModel(config, quant_config)
